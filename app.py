...
lineaccesstoken = 'U0qI9bEsqHW0aBQljT6jII+dATUOKqteYtJdI3b9IkweR2Iwsdvmdwfg0BwUDhcKfaTraQc+URS+GrJTAHZsDUEQXW9iV7I6yipOczwL8QlLdmHJxOhTqV7PMsdSuruL6YA6RVeM24xpJK6/TUg4PgdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดี" :
            replyObj = TextSendMessage(text="ไง")
        elif msg == "อยู่ที่ไหน" :
            replyObj = TextSendMessage(text="อยู่นี่ไงง")
        elif msg == "กินหมูทะกันมั้ย" :
            replyObj = TextSendMessage(text="หมูแพงนะ")
        else :
            replyObj = TextSendMessage(text="ไม่รู้สิ่นะ")
        line_bot_api.reply_message(rtoken, replyObj)
    elif msgType == "image":
        try:
